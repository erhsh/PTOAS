...
// Host driver for TileLang tinsert_acc ST — cube kernel smoke-test.
// Verifies matmul result via mte_l0c_gm output.
// tinsert behavior is smoke-test only (not numerically verified from GM).
...
void LaunchTINSERT_ACC_MAT_NZ_f32_16x16_idx0_0(uint16_t *a, uint16_t *b, float *out, void *stream);
...
static const TestCase kCases[] = {
    {"acc_mat_nz_f32_16x16_idx0_0", LaunchTINSERT_ACC_MAT_NZ_f32_16x16_idx0_0, 16, 16, 16, 16, 16, 16},
};
...
static int RunCase(const TestCase &tc, int deviceId, aclrtStream stream) {
    ...
    std::printf("[INFO] === case: %s (lhs=%zux%zu, rhs=%zux%zu, out=%zux%zu, tinsert=smoke-test) ===\n",
    ...
    if (rc == 0) {
        ...
        tc.launch(static_cast<uint16_t *>(lhsDevice), static_cast<uint16_t *>(rhsDevice), static_cast<float *>(outDevice), stream);
    ...
    if (lhsDevice != nullptr) aclrtFree(lhsDevice);
    if (rhsDevice != nullptr) aclrtFree(rhsDevice);
    if (outDevice != nullptr) aclrtFree(outDevice);
    if (lhsHost != nullptr) aclrtFreeHost(lhsHost);
    if (rhsHost != nullptr) aclrtFreeHost(rhsHost);
